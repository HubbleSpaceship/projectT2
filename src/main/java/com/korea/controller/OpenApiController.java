...
public class OpenApiController {
	 ...
	 @RequestMapping("/SelectOneCompany.do")
	   String SelectOneCompany(Model model, CompanyVO vo)  {
	 ...
	 @RequestMapping("/OpenSelectMap.do")
	   String OpenSelectMap(Model model, CompanyVO vo)  {
		   
		   model.addAttribute("li", service.list(vo))  ; 
		   
		   return "/company/open_selectAll.jsp";	   
	 ...
	 @RequestMapping("/CompanySelectAll.do")
	   String selectAll(Model model, CompanyVO vo)  {
	 ...
	 @RequestMapping("/EvSelectAll.do")
	   String selectAll(Model model, EvSearchVO vo)  {
		   
		   model.addAttribute("li", service.listEv(vo))  ; 
		   
		   return "/company/Ev_selectAll.jsp";	   
		
	   }
	 
	 
	 @RequestMapping("/EvSelectAllMap.do")
	   String EvSelectAllMap(Model model, EvSearchVO vo)  {
		   
		   model.addAttribute("li", service.listEv(vo))  ; 
		   
		   return "/company/Evopen_selectAllMap.jsp";	   
		
	   }
	 
	 @RequestMapping("/SelectOneEv.do")
	   String SelectOneEv(Model model, EvSearchVO vo)  {

		   model.addAttribute("m", service.EvSelectOne(vo));
		   return "/company/Ev_selectOne.jsp";	   
	 ...
	 @RequestMapping("/OpenApiInsert.do")
	   String OpenApiInsert(Model model, CompanyVO vo) throws Exception {
		 StringBuilder urlBuilder = new StringBuilder("http://apis.data.go.kr/6480000/gyeongnamgoodemploycompany/gyeongnamgoodemploycompanylist"); /*URL*/
		        urlBuilder.append("?" + URLEncoder.encode("serviceKey","UTF-8") + "=Rq98xJ3ext6KRpBRFRqKyCw5krJQ5GMFObCri%2F%2BWVP5%2FJQ9e4f6W0RD267VXCHk0UKWmHskX5KcueA5hgKh8Lw%3D%3D"); /*Service Key (일반인증키)*/
	        ...
	        try {
		         ...
		         Element item =(Element) items.getElementsByTagName("item").item(i);
			         ...
			         // 필요한 데이터 값 추출하기 
			         data1 = item.getElementsByTagName("entrprsNm").item(0); // 주소 
			         data2 = item.getElementsByTagName("rprsntvNm").item(0); // 회사명
			         data3 = item.getElementsByTagName("rdnmadr").item(0); // 회사대표
			         data4 = item.getElementsByTagName("logitude").item(0); // 주력상품
			         data5 = item.getElementsByTagName("latitude").item(0);  // 위도
			         data6 = item.getElementsByTagName("mainGoods").item(0);  // 경도 
			         ...
			         vo.setEntrprsNm(strData1);
			         vo.setRprsntvNm(strData2);
			         vo.setRdnmadr(strData3);
			         vo.setLogitude(strData4);
	 ...
	 @RequestMapping("/EvInsert.do")
	   String EvInsert(Model model, EvSearchVO vo) throws Exception {
		 StringBuilder urlBuilder = new StringBuilder("http://openapi.kepco.co.kr/service/EvInfoServiceV2/getEvSearchList"); /*URL*/
		        urlBuilder.append("?" + URLEncoder.encode("serviceKey","UTF-8") + "=Pz24WndiHwq%2F%2BK5gjSI5gB4YNhLZHFvhdiJGWNUOd7sgGNA9FMqDdXbHpBE5K5ZpOEd2xEKAb4hZieZmEji9IQ%3D%3D"); /*Service Key (일반인증키)*/
		        urlBuilder.append("&" + URLEncoder.encode("pageNo","UTF-8") + "=" + URLEncoder.encode("1", "UTF-8")); /*페이지 번호*/
	        ...
	        try {
		         ...
		         for(int i=1272 ; i<=3000 ; i++ ) {
		         Element item =(Element) items.getElementsByTagName("item").item(i);
		          
			         // 필요한 데이터 값 추출하기 
			         data1 = item.getElementsByTagName("csNm").item(0); // 주소 
			         data2 = item.getElementsByTagName("addr").item(0); // 회사명
			         data3 = item.getElementsByTagName("cpNm").item(0); // 회사대표
			         data4 = item.getElementsByTagName("cpStat").item(0); // 회사대표
